...
def parse_args():
    ...
    parser.add_argument('--name', default='model_test', help='model name: (default: arch+timestamp)')
...
class GetDataset(Dataset):
    def __init__(self, imageFolderDataset_ir, imageFolderDataset_vi, transform=None):
        self.imageFolderDataset_ir = imageFolderDataset_ir
        self.imageFolderDataset_vi = imageFolderDataset_vi
        self.transform = transform

    def __getitem__(self, index):
        ir_path = self.imageFolderDataset_ir[index]
        vi_path = self.imageFolderDataset_vi[index]
...
def train(args, train_loader_ir, model,
          criterion_ssim_ir, criterion_ssim_vi,
          criterion_RMI_ir, criterion_RMI_vi,
...
def main():
    ...
    training_dir_ir = "path/to/your IR image"  # 替换为IR图像文件夹路径
    folder_dataset_train_ir = sorted(glob.glob(os.path.join(training_dir_ir, '*.jpg')))

    training_dir_vi = "path/to/your VI image"  # 替换为VI图像文件夹路径
    folder_dataset_train_vi = sorted(glob.glob(os.path.join(training_dir_vi, '*.jpg')))
    ...
    dataset_train_ir = GetDataset(imageFolderDataset_ir=folder_dataset_train_ir,
                                  imageFolderDataset_vi=folder_dataset_train_vi,
                                  transform=transform_train)

    dataset_train_vi = GetDataset(imageFolderDataset_ir=folder_dataset_train_ir,
                                  imageFolderDataset_vi=folder_dataset_train_vi,
                                  transform=transform_train)
    ...
    for epoch in range(args.epochs):
        ...
        train_log = train(args, train_loader_ir, model,
                          criterion_ssim_ir, criterion_ssim_vi,
                          criterion_RMI_ir, criterion_RMI_vi,
                          optimizer, epoch)
        ...
        log.to_csv('model/%s/log.csv' %args.name, index=False)

        if (epoch+1) % 50 == 0:
            torch.save(model.state_dict(), 'model/%s/model_{}.pth'.format(epoch+1) %args.name)
